...
class ReposeRecord {
    ...
    void getAnswers(){
        ...
        int mostTimeAMinuteWasSlept
        int mostSleptMinute
        int[] minuteSleptDetails = guardWhoSleptTheMost.getMinutesSleptDetail()
        for(int i=0; i<60; i++) {
            if (minuteSleptDetails[i] > mostTimeAMinuteWasSlept) {
                mostTimeAMinuteWasSlept = minuteSleptDetails[i]
                mostSleptMinute = i
            }
        }
        ...
        // Finds the most slept minute by any guard
        String guardWithMostSleptMinuteEver
        for (Guard guard:guards.values()){
            minuteSleptDetails = guard.getMinutesSleptDetail()
            for(int i=0; i<60; i++) {
                if (minuteSleptDetails[i] > mostTimeAMinuteWasSlept) {
                    mostTimeAMinuteWasSlept = minuteSleptDetails[i]
                    mostSleptMinute = i
                    guardWithMostSleptMinuteEver = guard.getID()
                }
            }
        }
        println "Answer to phase 2 is: " + guardWithMostSleptMinuteEver + " x " + mostSleptMinute + " = " +  guardWithMostSleptMinuteEver.toInteger() * mostSleptMinute
    }

    private HashMap<Integer, Guard> processLogs() {
